...
#include <Core/Helpers/ProgressBar.h>
...
{
	m_pTextureMap = AddComponent<TextureMap>();
	m_pTransform = GetComponent<Transform3D>();
	GetComponent<MeshRenderer>()->SetByPointer(&m_Mesh);
}
...
void Landscape::Generate(const uInt& biomeCount) {
	...
	const Nt::Double2D basePosition(m_pTransform->Position().x, m_pTransform->Position().z);
	...
	GetComponent<Collider>()->SetShape(GetShape());
	...
	m_pTextureMap->Main.Create(4, vertexDensity, reinterpret_cast<Byte*>(pBiomeMap));
	m_pTextureMap->Main.BindUnit(1);
	for (auto& [biomeID, texture] : m_pTextureMap->Map)
...
std::unique_ptr<Nt::KDTree> Landscape::_CreateKDTree(const uInt& biomeCount) {
	...
	for (uInt i = 0; i < biomeCount; ++i) {
		...
		if (!m_pTextureMap->Map.contains(biomeID))
			m_pTextureMap->Map[biomeID].LoadFromFile(m_Biomes.back()->GetTexturePath());
	}
...
Nt::Double2D Landscape::_GenerateSite(const Double& minDistance) const noexcept {
	...
	std::uniform_real_distribution distX(terrainRect.Left, terrainRect.Right);
	std::uniform_real_distribution distY(terrainRect.Top, terrainRect.Bottom);
